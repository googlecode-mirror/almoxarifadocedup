...
class TCombo extends TField
{
	...
	public function setItemValues($valores){
		
		$this->itemValues = $valores;
    ...
    public function show()
    {
         ...
         if ($this->items)
         {
            ...
            foreach ($this->items as $chave => $item)
            {
                ...
                $option = new TElement('option');
                $option->value = (isset($this->itemValues)) ? $this->itemValues[$chave] : $chave; // define o índice da opção
                $option->add($item);     // adiciona o texto da opção
                // caso seja a opção selecionada
                if ($this->itemValues[$chave] == $this->selected)
                {
