	...
	foreach($campos as $campo => $itens){
		...
		if(!empty($_POST[$campo]) && array_key_exists('tipo',$itens)){
			...
			if($itens['tipo'] == 'string' && !preg_match("/^[a-zA-Z]+$/",$_POST[$campo])){
				$camposErrados[] = "No campo ".strtoupper($itens[0])." voce só pode digitar letras.<br/><br/>";
			}
			...
			if($itens['tipo'] == 'nome' && !preg_match("/^[a-z\d_]{4,28}$/i",$_POST[$campo])){
				$camposErrados[] = "O campo ".strtoupper($itens[0])." deve ter de 4 - 28 caracteres.<br/><br/>";
			}
			
			// Se for do tipo data, só aceitará data no formato dd/mm/yyyy
			if($itens['tipo'] == 'data' && !preg_match("/^\d{1,2}\/\d{1,2}\/\d{4}$/",$_POST[$campo])){
				$camposErrados[] = "O campo ".strtoupper($itens[0])." deve possuir o formato DD/MM/YYYY.<br/><br/>";
			}
			
			// Se for do tipo inteiro, só aceitará numeros
			if($itens['tipo'] == 'inteiro' && !preg_match("/^[0-9]+$/",$_POST[$campo])){
				$camposErrados[] = "No campo ".strtoupper($itens[0])." voce só pode digitar números.<br/><br/>";
			}
			...
			if($itens['tipo'] == 'igualdade' && strlen($_POST[$campo]) != strlen($_POST[$itens['compara']])){
				$camposErrados[] = "Os campos ".strtoupper($itens[0])." e ".strtoupper($itens['legenda_2'])." tem que ser iguais.<br/><br/>";
			}
			// valida o e-mail
			if($itens['tipo'] == 'email' && !preg_match("/^[a-z0-9_\.\-]+@[a-z0-9_\.\-]*[a-z0-9_\-]+\.[a-z]{2,4}$/i",$_POST[$campo])){
				$camposErrados[] = "Digite um E-mail válido no campo ".strtoupper($itens[0]).".<br/><br/>";
			}
			// valida o formato do telefone com DDD de 3 digitos
			if($itens['tipo'] == 'ddd_telefone' && !preg_match("/^\(\d{3}\)[\s-]?\d{4}-\d{4}$/",$_POST[$campo])){
				$camposErrados[] = "Digite o formato correto no campo ".strtoupper($itens[0]).", correto: (000)0000-0000.<br/><br/>";
			}
			...
			if($itens['tipo'] == 'ddd_3' && !preg_match("/^\d{3}$/",$_POST[$campo])){
				$camposErrados[] = "Digite o formato correto no campo ".strtoupper($itens[0]).", correto: 000.<br/><br/>";
			}
			// valida o numeros do telefone sem o DDD
			if($itens['tipo'] == 'telefone' && !preg_match("/^\d{4}-\d{4}$/",$_POST[$campo])){
				$camposErrados[] = "Digite o formato correto no campo ".strtoupper($itens[0]).", correto: 0000-0000.<br/><br/>";
			}
			// valida o CEP sem o traço
			if($itens['tipo'] == 'cep_num' && !preg_match("/^\d{5}$/",$_POST[$campo])){
				$camposErrados[] = "Digite o formato correto no campo ".strtoupper($itens[0])." correto: 00000000.<br/><br/>";
			}
			// valida o CEP com o traço
			if($itens['tipo'] == 'cep' && !preg_match("/^\d{5}-\d{3}$/",$_POST[$campo])){
				$camposErrados[] = "Digite o formato correto no campo ".strtoupper($itens[0])." correto: 00000-000.<br/><br/>";
			}
			// valida o tamanho do CPF
			if($itens['tipo'] == 'cpf_num' && !preg_match("/^\d{11}$/",$_POST[$campo])){
				$camposErrados[] = "Digite o formato correto no campo ".strtoupper($itens[0])." correto: 00000000000.<br/><br/>";
			}
			// valida o tamanho e o formato do CPF
			if($itens['tipo'] == 'cpf_separators' && !preg_match("/^\d{3}.\d{3}.\d{3}-\d{2}$/",$_POST[$campo])){
				$camposErrados[] = "Digite o formato correto no campo ".strtoupper($itens[0])." correto: 000.000.000-00.<br/><br/>";
			}
			// valida o tamanho do CNPJ, com o novo formato, de 15 digitos
			if($itens['tipo'] == 'cnpj_num' && !preg_match("/^\d{15}$/",$_POST[$campo])){
				$camposErrados[] = "Digite o formato correto no campo ".strtoupper($itens[0])." correto: 000000000000000.<br/><br/>";
			}
